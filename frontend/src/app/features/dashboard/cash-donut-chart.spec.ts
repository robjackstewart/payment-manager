...
import { CashDonutChartComponent } from './cash-donut-chart';
...
async function setup() {
  ...
  await TestBed.configureTestingModule({
    imports: [CashDonutChartComponent],
  }).compileComponents();

  const fixture = TestBed.createComponent(CashDonutChartComponent);
  const component = fixture.componentInstance;
  return { fixture, component, fakeChart };
}

describe('CashDonutChartComponent', () => {
  ...
  it('passes slices input as chart data', async () => {
    const { fixture } = await setup();
    const slices = [
      { label: 'Netflix', amount: 16 },
      { label: 'Left over', amount: 484 },
    ];
  ...
  it('renders as a donut via innerRadiusRatio on the pie series', async () => {
    const { fixture } = await setup();
    fixture.detectChanges();

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const createCall = vi.mocked(AgCharts.create).mock.calls[0][0] as any;
    expect(createCall.series[0].innerRadiusRatio).toBeGreaterThan(0);
  });
});
